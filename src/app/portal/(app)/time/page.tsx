...
export default async function PortalTimePage() {
  ...
  return <div className="space-y-6"><PageHeader title="Delivery time" description="A transparent view of time logged against your current allocation." /><Card><CardHeader><CardTitle className="text-base">Current period</CardTitle><CardDescription>{start.toLocaleDateString(undefined, { month: "long", year: "numeric" })}</CardDescription></CardHeader><CardContent className="grid gap-4 sm:grid-cols-3"><div><p className="text-xs text-muted-foreground">Logged</p><p className="font-mono text-2xl font-semibold tabular-nums">{formatLoggedTime(spent)}</p></div><div><p className="text-xs text-muted-foreground">Allocated</p><p className="font-mono text-2xl font-semibold tabular-nums">{budget ? formatLoggedTime(budget.allocatedSeconds) : "Not set"}</p></div><div><p className="text-xs text-muted-foreground">Remaining</p><p className="font-mono text-2xl font-semibold tabular-nums">{budget ? formatLoggedTime(Math.max(0, remaining)) : "—"}</p>{budget ? <Badge variant={remaining < 0 ? "destructive" : "secondary"}>{remaining < 0 ? "Over allocation" : "On track"}</Badge> : null}</div></CardContent></Card><Card><CardHeader><CardTitle className="text-base">Recent logged work</CardTitle></CardHeader><CardContent>{entries.length === 0 ? <EmptyState icon={Clock3} title="No time logged yet" description="Your team has not logged delivery time this period." /> : <div className="space-y-2">{entries.map((entry) => <div key={entry.id} className="flex items-center justify-between gap-3 border-b py-3 last:border-0"><div><p className="font-medium">{entry.taskId ? "Task work" : "Delivery work"}</p><p className="text-xs text-muted-foreground">{entry.projectId ? projectNames.get(entry.projectId) ?? "Project" : "General client work"} · {entry.startedAt.toLocaleDateString()}</p></div><Badge variant="outline" className="font-mono tabular-nums">{formatLoggedTime(entry.durationSeconds)}</Badge></div>)}</div>}</CardContent></Card></div>;
}
