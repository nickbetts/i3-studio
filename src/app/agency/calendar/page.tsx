...
export default async function AgencyCalendarPage() {
  ...
  const [members, rows] = await Promise.all([db.query.users.findMany({ where: (user, { inArray }) => inArray(user.role, ["admin", "account_manager", "content_writer"]), orderBy: desc(users.name) }), db.query.allocations.findMany({ orderBy: desc(allocations.date) })]);
