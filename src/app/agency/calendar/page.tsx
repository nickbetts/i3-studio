...
export default async function AgencyCalendarPage() {
  ...
  return <div className="space-y-6"><div><h1 className="text-2xl font-semibold">Team calendar</h1><p className="text-muted-foreground">Drag blocks to move them, pull the bottom edge to resize, or switch to month view.</p></div><Card><CardHeader><CardTitle className="text-base">Add time block</CardTitle><CardDescription>Blocks are managed by administrators. Time fields use minutes from midnight.</CardDescription></CardHeader><CardContent><form action={createAllocation} className="grid gap-4 md:grid-cols-5"><div className="space-y-2"><Label htmlFor="allocation-member">Team member</Label><Select name="memberUserId" required><SelectTrigger id="allocation-member"><SelectValue placeholder="Choose" /></SelectTrigger><SelectContent>{members.map((member) => <SelectItem key={member.id} value={member.id}>{member.name || member.email}</SelectItem>)}</SelectContent></Select></div><div className="space-y-2"><Label htmlFor="allocation-title">Work</Label><Input id="allocation-title" name="title" required /></div><div className="space-y-2"><Label htmlFor="allocation-date">Date</Label><Input id="allocation-date" name="date" type="date" required /></div><div className="space-y-2"><Label htmlFor="allocation-start">Start</Label><Input id="allocation-start" name="startMinute" type="number" min="0" max="1439" defaultValue="540" required /></div><div className="space-y-2"><Label htmlFor="allocation-end">End</Label><Input id="allocation-end" name="endMinute" type="number" min="1" max="1440" defaultValue="600" required /></div><div><Button type="submit">Add block</Button></div></form></CardContent></Card><CalendarGrid members={members.map((member) => ({ id: member.id, name: member.name || member.email }))} blocks={rows} /></div>;
}
