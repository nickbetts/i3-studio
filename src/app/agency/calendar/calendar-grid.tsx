...
export function CalendarGrid({ members, blocks }: { members: Member[]; blocks: Block[] }) {
  ...
  function move(id: string, event: React.DragEvent<HTMLDivElement>) {
    ...
    const startMinute = Math.max(0, Math.min(1439, Math.round(((event.clientY - bounds.top) / bounds.height) * 1440 / 15) * 15));
  ...
  function resize(id: string, event: React.PointerEvent<HTMLButtonElement>) {
    ...
    if (!block || !timeline) return;
    const startY = event.clientY;
    const initialEnd = block.endMinute;
    const onMove = (moveEvent: PointerEvent) => { const delta = Math.round(((moveEvent.clientY - startY) / timeline.getBoundingClientRect().height) * 1440 / 15) * 15; const endMinute = Math.max(block.startMinute + 15, Math.min(1440, initialEnd + delta)); setLocalBlocks((items) => items.map((item) => item.id === id ? { ...item, endMinute } : item)); };
