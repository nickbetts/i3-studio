...
async function main() {
  ...
  const isNewBrightleaf = !existingBrightleaf;
  const brightleafId =
  ...
  await db
    ...
    .onConflictDoNothing({ target: onboardingSubmissions.clientAccountId });

  if (!isNewBrightleaf) {
    console.log("Brightleaf demo data already exists — skipping re-seed of projects/documents/etc.");
  } else {
    // --- Projects + milestones -----------------------------------------
    ...
    await db.insert(tasks).values([
      { clientAccountId: acmeId, title: "Send brand assets", status: "open", priority: "medium" as const, createdByUserId: managerId },
    ...
    // --- Design assets + annotations ---------------------------------------
    const [approvedDesign] = await db
      .insert(designAssets)
      .values({ clientAccountId: brightleafId, createdByUserId: managerId, title: "Homepage hero concept", imageUrl: img("hero-concept"), status: "approved", createdAt: subDays(new Date(), 14) })
    ...
    await db.insert(designAssets).values({ clientAccountId: brightleafId, createdByUserId: managerId, title: "Email newsletter template", imageUrl: img("newsletter"), status: "changes_requested", createdAt: subDays(new Date(), 7) });
    ...
    // --- Content templates ---------------------------------------------------
    const [blogTemplate] = await db
      .insert(contentTemplates)
      .values({ name: "Blog post", contentType: "blog", fields: DEFAULT_TEMPLATE_FIELDS.blog, createdByUserId: adminId })
      .returning({ id: contentTemplates.id });
    const [webTemplate] = await db
      .insert(contentTemplates)
      .values({ name: "Web page copy", contentType: "webpage", fields: DEFAULT_TEMPLATE_FIELDS.webpage, createdByUserId: adminId })
      .returning({ id: contentTemplates.id });
    ...
    await createContentItem({
      ...
      comments: [{ fieldKey: "body", body: "Let's open with a concrete number — e.g. how many meals/families helped last year.", authorUserId: managerId, resolved: false, daysAgo: 4 }],
    });
    ...
    await db.insert(allocations).values([
      ...
      { memberUserId: adminId, clientAccountId: null, title: "Internal ops", date: addDays(weekStart, 3), endDate: addDays(weekStart, 3), startMinute: 720, endMinute: 1440, createdByUserId: adminId },
    ...
    await db.insert(auditLogs).values([
      ...
      { actorUserId: managerId, action: "design.uploaded", entityType: "design_asset", clientAccountId: brightleafId, createdAt: subDays(new Date(), 7) },
  ...
  console.log("Seed complete. Logins (password: password123 unless noted):");
