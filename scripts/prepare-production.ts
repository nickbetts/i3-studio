...
await database.transaction([
  ...
  database.query("CREATE TABLE IF NOT EXISTS task_assignment (id text PRIMARY KEY, task_id text NOT NULL REFERENCES task(id) ON DELETE CASCADE, user_id text NOT NULL REFERENCES \"user\"(id) ON DELETE CASCADE, created_at timestamptz NOT NULL DEFAULT now())"),
  database.query("CREATE UNIQUE INDEX IF NOT EXISTS task_assignment_unique_idx ON task_assignment (task_id, user_id)"),
  database.query("CREATE INDEX IF NOT EXISTS task_assignment_user_idx ON task_assignment (user_id, task_id)"),
  database.query("INSERT INTO task_assignment (id, task_id, user_id) SELECT gen_random_uuid()::text, id, assigned_to_user_id FROM task WHERE assigned_to_user_id IS NOT NULL ON CONFLICT (task_id, user_id) DO NOTHING"),
  ...
  database.query("ALTER TABLE time_entry ADD COLUMN IF NOT EXISTS service_type text NOT NULL DEFAULT 'account_manager_hours'"),
  database.query("ALTER TABLE active_timer ADD COLUMN IF NOT EXISTS service_type text NOT NULL DEFAULT 'account_manager_hours'"),
  database.query("CREATE TABLE IF NOT EXISTS client_service_allocation (id text PRIMARY KEY, client_account_id text NOT NULL REFERENCES client_account(id) ON DELETE CASCADE, period_start timestamptz NOT NULL, period_end timestamptz NOT NULL, service_type text NOT NULL, allocated_seconds integer NOT NULL DEFAULT 0, allocated_quantity integer NOT NULL DEFAULT 0, created_by_user_id text REFERENCES \"user\"(id) ON DELETE SET NULL, updated_at timestamptz NOT NULL DEFAULT now())"),
  database.query("CREATE UNIQUE INDEX IF NOT EXISTS client_service_allocation_period_idx ON client_service_allocation (client_account_id, period_start, service_type)"),
  database.query("CREATE INDEX IF NOT EXISTS client_service_allocation_client_idx ON client_service_allocation (client_account_id, period_start)"),
